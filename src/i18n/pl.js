export default {
  nav: {
    menu: "Menu",
    collections: "Kolekcje",
    shop: "Sklep",
    world: "Nasz świat",
    journal: "Dziennik",
    contact: "Kontakt",
    search: "Szukaj",
    account: "Konto",
    bag: "Torba",
    close: "Zamknij",
    men: "Mężczyźni",
    women: "Kobiety",
  },
  home: {
    hero: {
      label: "Zał. 2026 · Warszawa · 52°N",
      title: "ROJOB",
      subtitle: "Więcej niż ubrania. Wyższy standard na co dzień.",
      collection: "Pierwsza kolekcja",
      year: "2026",
      ctaDiscover: "Odkryj kolekcję",
      ctaLookbook: "Lookbook",
      ctaPrivate: "Dołącz do listy",
    },
    statement: {
      line1: "Urodzony w Warszawie.",
      line2: "Zdefiniowany na 52°N.",
      copy:
        "ROJOB to współczesna europejska marka ukształtowana przez Warszawę — miasto kontrastów, wytrwałości i nowoczesnej elegancji. Tworzymy trwałe elementy garderoby, w których spotykają się spokojna forma, precyzyjny detal i codzienny luksus.",
    },
    emblem: {
      label: "Znak rozpoznawczy",
      title: "Emblemat 52°N",
      subtitle: "Współrzędna geograficzna przekształcona w wizualny kod ROJOB.",
      warsaw: "Warszawa",
    },
    collection: {
      label: "Kolekcja",
      title: "Cable Cotton",
      code: "01 / 2026",
      copy:
        "Klasyczne dzianiny. Współczesne życie. Cable Cotton w Porcelain i Midnight — sfotografowane jak domowy lookbook, prezentowane jak działający sklep.",
      cta: "Zobacz kolekcję",
    },
    heroProduct: {
      label: "Kluczowy model",
      title: "Cable Half-Zip",
      copy:
        "Podniesiony sweter z połową zamka i splotem w warkocz, zaprojektowany jako stała warstwa garderoby. Charakterystyczny kołnierz, powściągliwe detale i emblemat ROJOB 52°N.",
    },
    material: {
      label: "Materiał",
      title: "Cable Cotton",
      copy:
        "Wyrafinowana interpretacja znanego splotu — stworzona do warstwowania, faktury i codziennej elegancji.",
      note: "100% bawełna",
    },
    warsaw: {
      label: "Pochodzenie",
      title: "Warszawa",
      coordinate: "52°N",
      copy:
        "Stworzony w Warszawie i ukształtowany przez kontrasty miasta — historię, nowoczesność, powściągliwość i ruch. ROJOB czerpie tożsamość z północnego światła i metropolitalnego charakteru, nie z pocztówek turystycznych.",
      coords: "52.3702° N · 21.0122° E",
    },
    detail: {
      label: "Rzemiosło",
      title: "Detal to tożsamość.",
      emblem: "Emblemat 52°N",
      emblemCopy: "Haftowany na każdym modelu z pierwszej kolekcji.",
      hardware: "Niestandardowy suwak R",
      hardwareCopy: "Metalowe wykończenie w modelu Cable Half-Zip.",
      label_: "Tkana metka",
      labelCopy: "ROJOB · Warszawa · 52°N",
    },
    colour: {
      label: "Paleta",
      porcelain: "Porcelain",
      crimson: "Warsaw Crimson",
      midnight: "Midnight",
      charcoal: "Charcoal",
    },
    campaign: {
      label: "Kampania",
      title: "Cichy europejski luksus",
    },
    keywords: {
      timeless: "Ponadczasowość",
      european: "Europejskość",
      modern: "Nowoczesność",
      authentic: "Autentyczność",
    },
    shop: {
      label: "Sklep",
      title: "Z pracowni",
      cta: "Wszystkie modele",
    },
    newsletter: {
      title: "Z Warszawy, pierwsi.",
      copy: "Prywatny dostęp do premier, historii i edycji ROJOB.",
      placeholder: "Adres e-mail",
      cta: "Dołącz do ROJOB",
    },
  },
  product: {
    addToBag: "Dodaj do torby",
    buyNow: "Kup teraz",
    added: "Dodano do torby",
    bag: "Torba",
    shippingNote: "Darmowa dostawa powyżej 500 zł · 30 dni na zwrot",
    oneSize: "Jeden rozmiar, dzianina swobodnie się rozciąga.",
    selectSize: "Wybierz rozmiar",
    colour: "Kolor",
    size: "Rozmiar",
    price: "Cena",
    preview: "Podgląd",
    unavailable: "Niedostępny",
    sizeGuide: "Tabela rozmiarów",
    details: "Szczegóły produktu",
    composition: "Skład",
    care: "Pielęgnacja",
    origin: "Pochodzenie",
    delivery: "Dostawa i zwroty",
    designedIn: "Zaprojektowano w Warszawie",
    madeIn: "Wyprodukowano w Bangladeszu",
    new: "Nowość",
    sale: "Promocja",
    collectionCode: "01/26",
    outOfStock: "Brak w magazynie",
    sizeOutOfStock: "Ten rozmiar jest niedostępny",
    outOfStockCopy: "Ten kolor jest wyprzedany. Wybierz inny, jeśli jest dostępny.",
    lowStock: "Ostatnie sztuki",
  },
  cart: {
    title: "Torba",
    empty: "Twoja torba jest pusta.",
    subtotal: "Suma częściowa",
    checkout: "Do kasy",
    viewBag: "Zobacz torbę",
    continue: "Kontynuuj zakupy",
    remove: "Usuń",
    qty: "Ilość",
  },
  footer: {
    tagline: "Wyższy standard na co dzień.",
    shop: "Sklep",
    collections: "Kolekcje",
    world: "Nasz świat",
    contact: "Kontakt",
    sizeGuide: "Tabela rozmiarów",
    delivery: "Dostawa i zwroty",
    terms: "Regulamin",
    privacy: "Polityka prywatności",
    cookies: "Pliki cookie",
    copyright: "© 2026 ROJOB",
    location: "Warszawa, Polska",
    newsletter: "Newsletter",
    language: "Język",
  },
  world: {
    title: "Nasz świat",
    est: "Zał. 2026 · Warszawa · 52°N",
    intro:
      "ROJOB to niezależna europejska marka modowa urodzona w Warszawie na 52°N — mieście kontrastów, wytrwałości i nowoczesnej elegancji.",
    coordinate: "Warszawa daje współrzędną. ROJOB definiuje język.",
    rojob: {
      title: "ROJOB",
      copy:
        "ROJOB to niezależna europejska marka modowa urodzona w Warszawie na 52°N. Założona w 2026 roku, dom rozwija trwałe elementy garderoby, w których spotykają się spokojna forma, precyzyjny detal i codzienny luksus — bez pożyczonego dziedzictwa ani sezonowej nowości dla samej nowości.",
    },
    fiftyTwo: {
      title: "52°N",
      copy:
        "Warszawa leży na około 52. stopniu szerokości geograficznej północnej. Emblemat ROJOB zamienia tę współrzędną w abstrakcyjny znak wizualny — dwa pola, jedna szczelina, nigdy flaga.",
    },
    warsaw: {
      title: "Warszawa",
      copy:
        "Północna stolica kamienia, stali i zimowego światła. Warszawa daje współrzędną. ROJOB definiuje język.",
    },
    design: {
      title: "Projekt",
      copy:
        "Zamiast podążać za sezonami dla samej nowości, ROJOB buduje garderobę trwałych form — ubrań, które mają stać się znajome, osobiste i długowieczne.",
    },
    material: {
      title: "Materiał",
      copy:
        "Pierwsza kolekcja koncentruje się na cable cotton — znanej strukturze reinterpretowanej z powściągliwością, proporcją i głębią dotyku.",
    },
    detail: {
      title: "Detal",
      copy:
        "Haftowane znaki, niestandardowe elementy metalowe i przemyślane opakowanie. Detal to tożsamość — nigdy dekoracja dla samej dekoracji.",
    },
    emblem: {
      title: "Emblemat",
      copy:
        "Warszawa leży na około 52. stopniu szerokości geograficznej północnej. Emblemat ROJOB zamienia tę współrzędną w abstrakcyjny znak wizualny.",
    },
  },
  fiftyTwo: {
    title: "52°N",
    warsaw: "Warszawa",
    headline: "52°N · Warszawa",
    intro:
      "Warszawa leży na około 52. stopniu szerokości geograficznej północnej. Emblemat ROJOB zamienia tę współrzędną w abstrakcyjny znak wizualny.",
    left: "Lewo · 52°",
    right: "Prawo · N",
    gap: "Przezroczyste centrum",
    construction: {
      label: "Konstrukcja",
      title: "Jak zbudowany jest emblemat",
    },
    colours: {
      upper: "Górne pole · Porcelain",
      lower: "Dolne pole · Warsaw Crimson",
      lettering: "Litery · Charcoal",
    },
    note:
      "Dwa pionowe pola w Porcelain i Warsaw Crimson, rozdzielone realną przezroczystą szczeliną. Litery w kolorze Charcoal. Oryginalny znak geograficzny — nie flaga narodowa.",
  },
  journal: {
    title: "Dziennik",
    subtitle: "Historie z Warszawy i pracowni",
    notFound: "Artykuł nie znaleziony",
    notFoundCopy: "Tego artykułu nie ma w dzienniku. Wróć, aby przeczytać inne historie z Warszawy i pracowni.",
    back: "Wróć do dziennika",
    categories: {
      warsaw: "Warszawa",
      objects: "Obiekty",
      material: "Materiał",
      people: "Ludzie",
      fiftyTwo: "52°N",
      collections: "Kolekcje",
    },
    read: "Czytaj",
  },
  contact: {
    title: "Kontakt",
    customerCare: "Obsługa klienta",
    marketing: "Marketing",
    ceo: "CEO",
    phone: "Telefon",
    press: "Prasa",
    wholesale: "Hurt",
    general: "Zapytania ogólne",
    emailPlaceholder: "info@rojob.eu",
    emails: {
      customerCare: "info@rojob.eu",
      press: "info@rojob.eu",
      wholesale: "info@rojob.eu",
      general: "info@rojob.eu",
      marketing: "marketing@rojob.eu",
      ceo: "ceo@rojob.eu",
    },
    note: "Odpowiadamy w ciągu dwóch dni roboczych.",
    form: {
      department: "Dział",
      name: "Imię i nazwisko",
      email: "E-mail",
      message: "Wiadomość",
      send: "Wyślij zapytanie",
      sending: "Wysyłanie…",
      sent: "Zapytanie otrzymane",
      sentCopy: "Wiadomość została wysłana na info@rojob.eu. Odpowiemy na podany adres e-mail.",
      sendAnother: "Wyślij kolejną wiadomość",
      sendError: "Nie udało się wysłać wiadomości. Napisz bezpośrednio na info@rojob.eu.",
      configError: "Wysyłka e-mail nie jest skonfigurowana. Napisz bezpośrednio na info@rojob.eu.",
    },
  },
  sizeGuide: {
    title: "Tabela rozmiarów",
    howTo: "Jak mierzyć",
    chest: "Klatka piersiowa",
    length: "Długość",
    sleeve: "Rękaw",
    shoulder: "Barki",
    placeholder: "Wymiary do potwierdzenia.",
    placeholderCell: "[Do potwierdzenia]",
    note: "ROJOB stosuje standardowe międzynarodowe rozmiarówki odzieży. Wszystkie wymiary podano dla produktu rozłożonego na płasko, w centymetrach, z tolerancją do 2 cm.",
    conversions: "Przeliczniki międzynarodowe",
    garmentFlat: "Wymiary produktu na płasko (cm)",
    bodyChest: "Obwód klatki piersiowej",
    accessories: "Akcesoria",
    accessoriesNote: "Czapki i szaliki mają jeden rozmiar, a dzianina swobodnie się rozciąga.",
    fit: "Krój",
    measure: {
      chest: "Zmierz wokół najszerszej części klatki piersiowej, trzymając taśmę poziomo i luźno — bez ściskania.",
      length: "Zmierz od najwyższego punktu ramienia (przy szyi) prosto w dół do pożądanego dołu.",
      sleeve: "Zmierz od szwu ramienia wzdłuż zewnętrznej strony rękawa do mankietu, z lekko zgiętym ramieniem.",
      shoulder: "Zmierz prosto przez plecy od punktu ramienia do punktu ramienia.",
    },
  },
  wholesale: {
    label: "Handel",
    title: "Hurt",
    seo: "ROJOB — hurt, private label i produkcja OEM dzianin dla sklepów, marek i dystrybutorów.",
    heroHeadline: "Produkcja dzianin premium dla nowoczesnych marek",
    heroCopy:
      "Hurt, private label i produkcja OEM — dziane w Warszawie, z powtarzalną jakością i nastawieniem na długą współpracę.",
    intro:
      "ROJOB dostarcza wybrane sklepy i partnerów na warunkach handlowych. Ta strona jest wyłącznie dla przedsiębiorców. Konsumenci powinni korzystać ze sklepu i kontaktu na stronie głównej.",
    applyTitle: "Wniosek o dostęp hurtowy",
    applyCopy:
      "Opiszcie Państwo działalność. Zatwierdzenie jest uznaniowe. Złożenie wniosku nie tworzy umowy dostawy.",
    applyCta: "Poproś o wycenę",
    shopCta: "Zobacz sklep",
    viewTerms: "Zobacz regulamin hurtowy",
    closeTerms: "Zamknij",
    aboutLabel: "O współpracy handlowej",
    aboutTitle: "Rzemiosło i jasne warunki",
    aboutCopy:
      "Dziejemy na 52°N, w warszawskiej pracowni, która od lat tworzy ubrania mające przetrwać więcej niż jeden sezon. Ta sama hala, przędza i te same ręce, które robią kolekcję detaliczną, realizują każde zamówienie hurtowe.",
    aboutCopyTwo:
      "Niezależnie od tego, czy jesteście młodą marką zamawiającą pierwszą kapsułę, czy siecią planującą zakup sezonowy — dostajecie moce zakładu produkcyjnego i uwagę małego studia.",
    servicesLabel: "Co oferujemy",
    servicesTitle: "Kompleksowa produkcja dzianin",
    servicesCopy:
      "Hurt z magazynu, private label i produkcja na zamówienie dla marek, sklepów i dystrybutorów szukających partnera produkcyjnego, a nie dostawcy.",
    whyLabel: "Dlaczego ROJOB",
    whyTitle: "Partner produkcyjny, który dotrzymuje terminu",
    whyCopy:
      "Kupujący wracają do nas, bo druga dostawa wygląda dokładnie jak pierwsza i bo zawsze ktoś odbiera telefon.",
    processLabel: "Jak to działa",
    processTitle: "Od zapytania do dostawy",
    processCopy:
      "Uporządkowany proces potwierdzany na piśmie. Zawsze wiecie, na jakim etapie jest zamówienie i co dzieje się dalej.",
    splitLabel: "Private label",
    splitTitle: "Wasza marka, nasza produkcja",
    splitCopy:
      "Sklepy i dystrybutorzy mogą kupować kolekcję ROJOB w cenach hurtowych. Marki mogą korzystać z tej samej produkcji pod własnym szyldem — bez żadnych oznaczeń ROJOB na wyrobie, opakowaniu ani dokumentacji.",
    faqLabel: "Pytania",
    faqTitle: "Najczęstsze pytania",
    formLabel: "Kontakt",
    termsTeaser: "Przeczytaj pełne warunki hurtowe przed złożeniem wniosku. Wymagane do dostępu handlowego.",
    termsLink: "Hurtowe Warunki Handlowe",
    acceptTermsPrefix: "Potwierdzam, że działam jako przedsiębiorca i akceptuję",
    fields: {
      company: "Nazwa firmy",
      nip: "NIP / VAT",
      contactName: "Osoba kontaktowa",
      email: "E-mail służbowy",
      phone: "Telefon",
      country: "Kraj",
      website: "Strona / sklep",
      productType: "Rodzaj produktu",
      productTypePlaceholder: "Wybierz rodzaj produktu",
      quantity: "Szacowana ilość",
      message: "O działalności",
    },
    submit: "Wyślij zapytanie",
    sending: "Wysyłanie…",
    successTitle: "Otrzymano",
    success:
      "Wniosek otrzymany. Jeśli będziemy mogli kontynuować, skontaktujemy się pod podanymi danymi.",
    termsTitle: "Hurtowe warunki handlowe",
    termsEffective: "Obowiązuje od 11 września 2026 · wyłącznie B2B",
    legalNote:
      "Warunki maksymalizują ochronę umowną ROJOB w praktyce B2B. Nie wyłączają odpowiedzialności, której prawo bezwzględnie nie pozwala wyłączyć. Przed użyciem w obrocie wymagają przeglądu prawnego i uzupełnienia danych firmy.",
  },
  legal: {
    nav: "Informacje prawne",
    terms: "Regulamin",
    privacy: "Polityka prywatności",
    cookies: "Polityka plików cookie",
    delivery: "Dostawa",
    deliveryReturns: "Dostawa i zwroty",
    returns: "Zwroty",
    complaints: "Reklamacje",
    payments: "Płatności",
    company: "Informacje o firmie",
    reviewNote: "Wymaga weryfikacji prawnej przed publicznym uruchomieniem.",
    placeholders: {
      company: "[NAZWA PRAWNA FIRMY]",
      nip: "[NIP]",
      regon: "[REGON/KRS]",
      address: "[ADRES REJESTROWY]",
      email: "info@rojob.eu",
    },
    bodies: {
      terms:
        "Niniejszy regulamin określa zasady korzystania ze strony ROJOB oraz zakupu towarów od [NAZWA PRAWNA FIRMY] (\"ROJOB\", \"my\"). Składając zamówienie lub korzystając z witryny, akceptujesz te warunki.\n\nWszystkie opisy produktów, zdjęcia i ceny podawane są w dobrej wierze. ROJOB zastrzega sobie prawo do zmiany informacji o produkcie przed wysyłką. Zamówienia podlegają akceptacji i dostępności.\n\nW sprawach regulaminu: [ADRES E-MAIL KONTAKTOWY]. Siedziba: [ADRES REJESTROWY]. NIP: [NIP]. REGON/KRS: [REGON/KRS].",
      privacy:
        "ROJOB szanuje Twoją prywatność. Niniejsza polityka wyjaśnia, w jaki sposób [NAZWA PRAWNA FIRMY] zbiera i przetwarza dane osobowe, gdy odwiedzasz rojob.eu, zakładasz konto, zapisujesz się do newslettera lub kontaktujesz się z nami.\n\nPrzetwarzamy dane na podstawach prawnych obejmujących wykonanie umowy, prawnie uzasadniony interes oraz zgodę (gdy wymagana). Przysługują Ci prawa wynikające z RODO — w tym dostęp, sprostowanie, usunięcie, ograniczenie, przenoszenie i sprzeciw. Aby je wykonać, napisz na [ADRES E-MAIL KONTAKTOWY].\n\nPrzechowujemy dane osobowe tylko tak długo, jak jest to konieczne. Nie sprzedajemy Twoich danych podmiotom trzecim.",
      cookies:
        "Ta witryna używa niezbędnych plików cookie wymaganych do bezpieczeństwa, preferencji językowych i funkcji sklepu. Opcjonalne pliki analityczne stosujemy wyłącznie za Twoją zgodą w banerze cookie.\n\nMożesz wycofać zgodę w dowolnym momencie, zmieniając ustawienia przeglądarki lub preferencje cookie na tej stronie. Szczegóły przetwarzania danych związanych z plikami cookie znajdziesz w Polityce prywatności.",
      deliveryReturns:
        "Zamówienia wysyłamy z Warszawy w ciągu 1–2 dni roboczych, od poniedziałku do piątku. Link do śledzenia przesyłki otrzymasz e-mailem, gdy paczka od nas wyjedzie.\n\nCzas i koszt dostawy: Polska 1–2 dni robocze, gratis powyżej 500 PLN, poniżej 19 PLN. Unia Europejska 2–5 dni roboczych, gratis powyżej 120 EUR, poniżej 25 EUR. Wielka Brytania, Szwajcaria i Norwegia 3–7 dni roboczych za 35 EUR, cło płatne przy odbiorze. Pozostałe kraje 5–10 dni roboczych za 45 EUR, cło i podatki płatne przy odbiorze.\n\nZwroty przyjmujemy w ciągu 30 dni od dostawy; na terenie Polski i Unii Europejskiej są bezpłatne. Jako konsument w UE masz dodatkowo ustawowe prawo odstąpienia od umowy w ciągu 14 dni od otrzymania towaru, bez podania przyczyny.\n\nZwracane produkty muszą być nienoszone, nieprane oraz odesłane ze wszystkimi metkami i oryginalnym opakowaniem. Zwrot środków realizujemy na pierwotną metodę płatności w ciągu 14 dni od otrzymania przesyłki zwrotnej. Wymiana rozmiaru jest bezpłatna w Polsce i UE, w miarę dostępności.\n\nAby zgłosić zwrot lub wymianę, napisz na [ADRES E-MAIL KONTAKTOWY], podając numer zamówienia. Niniejsza polityka nie narusza ustawowych uprawnień konsumenta wynikających z prawa polskiego i unijnego. [NAZWA PRAWNA FIRMY], [ADRES REJESTROWY].",
      payments:
        "Akceptowane metody płatności i szczegóły bezpiecznej kasy zostaną wymienione przed rozpoczęciem transakcji na żywo. ROJOB będzie korzystać z procesorów płatności zgodnych z PCI.\n\nCeny wyświetlane są w walucie wybranej na stronie. Ostateczne podatki i cła zależą od kraju dostawy i zostaną potwierdzone przy kasie.",
      complaints:
        "Jeśli chcesz złożyć reklamację dotyczącą produktów lub usług ROJOB, skontaktuj się z [ADRES E-MAIL KONTAKTOWY], podając numer zamówienia i opis problemu.\n\n[NAZWA PRAWNA FIRMY] potwierdzi otrzymanie reklamacji w ciągu [DO POTWIERDZENIA] dni roboczych i odpowie zgodnie z obowiązującymi przepisami o ochronie konsumentów w Polsce i Unii Europejskiej.",
      company:
        "[NAZWA PRAWNA FIRMY] to podmiot prawny prowadzący markę ROJOB.\n\nAdres rejestrowy: [ADRES REJESTROWY]\nNIP: [NIP]\nREGON/KRS: [REGON/KRS]\nKontakt: [ADRES E-MAIL KONTAKTOWY]\n\nROJOB to niezależna europejska marka modowa założona w Warszawie w 2026 roku.",
    },
  },
  cookie: {
    title: "Pliki cookie",
    copy:
      "Używamy niezbędnych plików cookie do działania witryny. Opcjonalne pliki analityczne stosujemy wyłącznie za Twoją zgodą.",
    accept: "Akceptuj",
    reject: "Odrzuć opcjonalne",
    settings: "Ustawienia",
  },
  newsletter: {
    title: "Newsletter",
    copy: "Prywatny dostęp do premier, historii i edycji ROJOB.",
    placeholder: "Adres e-mail",
    cta: "Dołącz do ROJOB",
    success: "Dziękujemy. Jesteś na liście.",
  },
  account: {
    title: "Konto",
    orders: "Zamówienia",
    addresses: "Adresy",
    wishlist: "Lista życzeń",
    profile: "Profil",
    forgotPassword: "Nie pamiętasz hasła?",
    resetNeedsEmail: "Najpierw podaj adres e-mail, a potem poproś o link resetujący.",
    resetSent: "Jeśli konto dla tego adresu istnieje, link resetujący jest już w drodze. Sprawdź także folder spam.",
    signIn: "Zaloguj się",
    signOut: "Wyloguj się",
    create: "Utwórz konto",
    password: "Hasło",
    name: "Imię i nazwisko",
    phone: "Telefon",
    address: "Adres",
    save: "Zapisz profil",
    saved: "Profil zapisany.",
    intro: "Zaloguj się, aby zapisać koszyk, listę życzeń i dane dostawy na wszystkich urządzeniach.",
    ordersEmpty: "Brak zamówień. Historia pojawi się po pierwszym zakupie.",
    prelaunchNote: "Utwórz konto, aby zsynchronizować koszyk, listę życzeń, telefon i adres z ROJOB.",
    profileNote: "Dodaj dane Firebase, aby włączyć konta w chmurze. Lista życzeń jest nadal zapisywana na tym urządzeniu.",
    ordersPlaceholder: "Brak zamówień. Historia pojawi się po pierwszym zakupie.",
    addressesPlaceholder: "Zapisane adresy pojawią się po uruchomieniu kasy.",
    wishlistEmpty: "Twoja lista życzeń jest pusta.",
    authError: "Nie udało się zalogować. Sprawdź dane logowania.",
  },
  search: {
    title: "Szukaj",
    placeholder: "Szukaj produktów, kolekcji, artykułów…",
    noResults: "Brak wyników.",
    products: "Produkty",
    collections: "Kolekcje",
    stories: "Historie",
  },
  common: {
    loading: "Ładowanie",
    error: "Coś poszło nie tak.",
    back: "Wróć",
    viewAll: "Zobacz wszystko",
    learnMore: "Dowiedz się więcej",
    close: "Zamknij",
    en: "EN",
    pl: "PL",
    currency: "Waluta",
    prelaunch: "Podgląd",
    commerce: "Kup teraz",
  },
};
